...
func StartIdleEventSubscriber(ctx context.Context) {
	...
	pubsub := rdbClient.Subscribe(ctx, "idle_events")
	ch := pubsub.Channel()
	go func() {
		log.Println("[WS] idle_events subscriber started")
		for msg := range ch {
			log.Printf("[WS] idle_event raw payload: %s", msg.Payload)
			var payload map[string]interface{}
			if err := json.Unmarshal([]byte(msg.Payload), &payload); err != nil {
				log.Printf("[WS] invalid idle event payload: %v", err)
				continue
			}

			// Expected payload types: player_idle_warning, player_forfeit
			...
			log.Printf("[WS] idle_event received: type=%s game_id=%s", typeStr, gameID)
